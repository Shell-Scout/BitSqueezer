...
def compress_png_batch(batch):
    try:
        ...
        if result.returncode != 0:
            return [f"[PNG] Error compressing batch: {result.stderr.decode(errors='ignore')}"]
    except Exception as e:
        return [f"[PNG] Exception: {e}"]
    return [None]*len(batch)

print(f"🖼 Compressing PNG ({len(png_files)} files)...")
...
with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
    ...
    for future in tqdm(as_completed(futures), total=len(futures), desc="PNG Compression", unit="batch"):
        res = future.result()
...
print(f"📦 Packing compressed files into {OUTPUT_ZIP}...")
...
print(f"\n✅ Done! Compressed archive: {OUTPUT_ZIP}")
print(f"⏱ Execution time: {round(duration, 2)} seconds")

if errors:
    print(f"⚠️ Errors occurred. Check {ERROR_LOG} for details.")
